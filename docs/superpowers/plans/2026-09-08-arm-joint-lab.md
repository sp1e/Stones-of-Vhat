...
> **Execution addendum, 2026-09-08:** The original one-native-step arm failed independent user-selected handoff-phase stress. The explicit addendum immediately below supersedes that integration detail only. Preserve the original plan and all original successful/failed control measurements as historical evidence; do not replay its earlier snippets over the current implementation. See [execution evidence](2026-09-08-arm-lab-results.md). Core identity/pose contracts and the normal Grip yard remain unchanged.

> **Executed configuration:** The first four-native-step candidate also failed dense phase 264. The preregistered fallback was implemented: **eight native 1/480 physical steps per authoritative 1/60 tick**, eight solver iterations /maxCcdSubsteps four /CCD on; bootstrap/animation remain one native 1/60 step. Final independent SPEC PASS and QUALITY APPROVE include a dense 759-distinct-phase supplementary diagnostic. Original candidate examples below are historical, not instructions to revert the accepted constant. Two core API comments were clarified; their behavior remains unchanged.

## Reviewed physical integration correction

Parent reviewed the sole writer's exact proposal after 55-phase RED and the failed bounded solver/CCD alternatives. No gate widening, hidden phase restriction, body/geometry change, dependency change or engine decision is authorized. This is an explicit integrator revision inside the isolated arm, not a change to the approved gameplay design.

- Bootstrap and animation remain one native `world.step()` at 1/60 s. The existing authority's adjacent history and COM velocity sample duration remain 1/60 s; no new authority or intermediate animation target API is introduced.
- After a validated successful handoff, physical integration uses **four native 1/240 s steps inside one authoritative 1/60 s command/publication tick**. Candidate configuration is eight solver iterations per native step, maxCcdSubsteps four, default PGS one and per-body CCD enabled. All are arm-only; native calls and dt are asserted, not inferred from visual speed. If this candidate fails, the only further preregistered subdivision attempt is eight native 1/480 s steps with the same settings and correspondingly changed explicit count tests. No unbounded tuning is implied.
- Compute the existing outer torque waveform/sign and parent-joint world frame once at the start of each physical outer tick. Divide that unchanged full outer world-space torque impulse into four equal pairs, applied before the four native steps. Do not resample the waveform/frame inside the batch. Optional point impulse remains a single authority-handoff operation before the batch; it is never reapplied by an inner step.
- Accrue the torque-work allowance from speeds immediately before/after each quarter-impulse pair, before native integration/contact. After **every** native step, measure anchor, penetration, joint-frame projections, finite state, speeds, energy and real contact/impulse data. All locked physical gates apply to every sample, not only the final endpoint. Current endpoint metrics keep their original meaning.
- Only after every native step succeeds may the owner increment its authoritative tick, capture authority once and publish one completed 1/60 interval. A setter, native step or measurement failure at any inner position destroys the fixture/publisher; no half-completed interval is published.
- Add a detached `lastPhysicsStep` record: null before the first physical batch; afterward `{nativeDtS, nativeSteps, samples:[{metrics, energyAllowanceJ}]}`. It describes the last completed batch only. Snapshot mutation cannot alter owned samples. Pause preserves the completed record, while reset/destroy discards it.
- Contact history units become explicit: `wallTicks` counts authoritative outer ticks with any wall contact, `wallNativeSteps` counts native steps with wall contact, and `wallImpulseNs` sums native normal impulses once per native step. Endpoint solver-contact counts are not cumulative history.
- Retain the original unstepped 4-iteration/budget-four limited/unlimited controls and bad-default-budget control unchanged. Add a **matched** limited/unlimited pair with the revised integration/quarter-impulse schedule and identical new settings. Do not silently refresh initial mass caches; the first-quarter behavior belongs to the new integrator, not a byte-identical replay of the historical control.

Test-first corrections: native count/dt and total simulated time; detached four-sample publication; point impulse once across a batch; all-sample 55-phase physical bounds; injected failure on the third native step leaving zero bodies/colliders/joints and invalid reads; existing 30/60/144 replay and pause semantics. Independent quality also samples held-out neighborhoods of failing phases and longer animation durations before acceptance. A sampled phase matrix is empirical fixture acceptance, not proof for all possible phases, full anatomy or an arbitrary-duration humanoid.

Parent authorizes the sole arm writer to implement this bounded addendum after reviewing it. Final independent spec recheck, subsequent quality recheck, coordinator strict/native/build/browser checks and actual screenshots remain required. The original exact plan follows for provenance.
