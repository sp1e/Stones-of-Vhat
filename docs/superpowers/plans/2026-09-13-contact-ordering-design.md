...
An authoritative family has exactly one result for each expected blocker and identical world epoch, adjacent interval ticks, cast ID and projectile ID. Unknown, duplicate, missing or mixed identities reject before publishing an ordering. A caller-provided subset must not masquerade as a complete world query. Full-city spatial candidates and a query-local coordinate frame remain separate prerequisites; the compiler's current limits must not reshape Claude's environment.
...
Let U be the minimum confirmed upper contact time among hits/initial blocks. If there is no such upper time, U is conceptually unbounded for this comparison (do not serialize Infinity as output data).
...
- Pair queries already cap at128 native calls; the whole family also needs a documented total work ceiling. Merely multiplying the maximum128 colliders by128 calls is a bound, not frame-time acceptance. Measure actual lab and stress cases before selecting a lower global budget. Exhaustion must retain unqueried blockers as unresolved from time0, never permit a later convenient hit.
...
- The eventual consumer revalidates current epoch, active cast, target collider and anatomy revision immediately before committing one effect. A stale contact cannot recreate an amputated limb or damage its former owner. No damage graph is introduced only to test this query layer.
